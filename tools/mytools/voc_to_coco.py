# 网上找了半天的资料都是废话太多，现在就想把pascal数据集转成coco数据集, 下面代码轻松快捷，改一下pascal访问地址就行。
PRE_DEFINE_CATEGORIES = {"backpack": 1, "satchel": 2, "trolley case": 3, "tote bag": 4}
...
class voc2coco:
    def __init__(self, devkit_path=None, year=None):
        self.classes = ('__background__',
                        'backpack', 'satchel', 'trolley case', 'tote bag')
    ...
    def _load_annotation(self, ids=[]):
        ...
        annotation_id = 1
        for index in ids:
            # index = str(index).replace("_","").replace('metro','1').replace('station','2').replace('street','3').replace('streetnight','4').replace('market','5')
            ...
            if os.path.exists(json_file):
                ...
                for shape in seg_data['shapes']:
                    ...
                    one_ann_msg = {"segmentation": [seg_msg],
                                   ...
                                   "iscrowd": 0,
                                   "image_id": int(str(index).replace("_","").replace('metro','1').replace('station','2').replace('streetnight','4').replace('street','3').replace('market','5')),
            ...
            one_image_msg = {"file_name": filename + ".jpg",
                             "height": int(height),
                             "width": int(width),
                             "id": int(str(index).replace("_","").replace('metro','1').replace('station','2').replace('streetnight','4').replace('street','3').replace('market','5'))
                             }
...
def demo():
    ...
    converter = voc2coco('D:/资料/CV/video/VOCdevkit', '2007')
